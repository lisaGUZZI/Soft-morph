...
class SoftClosing(nn.Module):
    ...
    def forward(self,input_img, iterations, dilation_connectivity = 4, erosion_connectivity = 4):
        """
        Inputs :
        - im : input 2D image of shape [batch_size, channels, height, width] or [height, width].
...
class SoftSkeletonizer(nn.Module):
    ...
    def allcondArithm(self, n):
        ...
        """
        F1 = (1 - n[:, :, :, 0])
        F2 = ((1 - n[:, :, :, 1]) * (1 - n[:, :, :, 7]) * (1-n[:, :, :, 2]-n[:, :, :, 3]+2*n[:, :, :, 2]*n[:, :, :, 3]-n[:, :, :, 4]+2*n[:, :, :, 2]*n[:, :, :, 4]+2*n[:, :, :, 3]*n[:, :, :, 4]-4*n[:, :, :, 2]*n[:, :, :, 3]*n[:, :, :, 4]) *
